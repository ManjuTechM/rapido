...
from rapido.models import Category,Location,Vendor,Vehicle,Chauffeur,Company,Bookings,Dutyslip,Invoice
...
from rapido.models import Location,Vendor,Vehicle,Chauffeur,Company,Bookings,Dutyslip,Invoice,Bookingstatus,Dutyslipstatus,Invoicestatus,Billingbase,City_ratecard,Outstation_ratecard,Airport_ratecard,Company_city_ratecard,Company_outstation_ratecard,Company_airport_ratecard
...
def sendbookingemail(request,bookingid):
    ...
    cc =['pramod.cr@rapidotravels.co.in','jins@rapidotravels.co.in','rajesh.chandran@rapidotravels.co.in']
...
def sendcancelemail(request,bookingid):
    ...
    html_content = render_to_string('rapido/cancel.html', context=context).strip()
    
    guest_email = booking.booking.guest_email
    booker_email = booking.booking.booking_email

    subject = 'Cancellation of Booking:'+'RTIPL-'+booking.booking.servicing_city.city_code+'-'+booking.booking.booking_id
    from_email = 'bookings@rapidotravels.co.in'
    reply_to = ['bookings@rapidotravels.co.in']
    to = [guest_email,booker_email]
    cc =['pramod.cr@rapidotravels.co.in','jins@rapidotravels.co.in','rajesh.chandran@rapidotravels.co.in']
    ...
    if request.method == 'POST':
        ...
        if user is not None:
            if user.is_active:
                ...
                if user.is_staff:
                	return render_to_response('rapido/staff/staff.html', context)
                else:
...
def admin_dutyslip_list(request):
    ...
    context_dict['cat_list'] = cat_list
    user = request.user
    booking_list= Bookings.objects.filter(status=1)
